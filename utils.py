...
def split_train_test(X: pd.DataFrame, y: pd.Series,
    ...
    train_set_to_return = X.sample(frac=train_proportion)
    subset_tests_rows_indexes = X.index.difference(train_set_to_return.index)
    test_set_to_return = X.loc[subset_tests_rows_indexes]
    return train_set_to_return, y.loc[
        train_set_to_return.index], test_set_to_return, y.loc[
        test_set_to_return.index]
